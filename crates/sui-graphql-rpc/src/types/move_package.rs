...
use crate::context_data::package_cache::{
    Error as PackageCacheError, Package as ParsedMovePackage,
};
...
impl MovePackage {
    fn parsed_package(&self) -> Result<ParsedMovePackage, Error> {
        ...
        ParsedMovePackage::read(
            super_.native.id().into(),
            super_.native.version().value(),
            &super_.stored.serialized_object,
        )
